...
ROUTE_KEYWORDS = {
    "VAC-VIDEO-001": [
        "影片", "短影音", "剪輯", "字幕", "逐字稿", "配樂", "mp4", "video", "edit"
    ],
    "VAC-SLIDE-001": [
        "簡報", "投影片", "ppt", "pptx", "slides", "deck", "講義"
    ],
    "VAC-WEB-001": [
        "網站", "網頁", "html", "css", "javascript", "rwd", "landing page", "web"
    ],
    "VAC-DATA-001": [
        "數據", "資料分析", "excel", "csv", "統計", "圖表", "kpi", "data"
    ],
    "VAC-REPORT-001": [
        "報告", "docx", "正式文件", "會議紀錄", "摘要", "研究報告", "report"
    ],
}
...
def plan(card: dict[str, Any]) -> dict[str, Any]:
    critical_constraints = [
        item.get("rule")
        for item in card.get("constraints", [])
        if item.get("severity") == "critical"
    ]
    required_tools = [
        item
        for item in card.get("tools_capabilities", [])
